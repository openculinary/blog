...
Although inter-service communication within RecipeRadar typically uses JSON to represent objects and data, JSON is not well suited to representing text with inline metadata.  Instead, we've found that XML is a useful alternative since it enables us to add markup that is attached to individual phrases.
